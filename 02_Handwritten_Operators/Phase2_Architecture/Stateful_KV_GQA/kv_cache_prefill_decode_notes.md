# KV Cache 与 Prefill/Decode 两阶段推理知识笔记

> 关联文档：[Stateful_KV_GQA/README.md](README.md) · [../FlashAttention/README.md](../FlashAttention/README.md)
> 关联代码：[gqa_attention_base.py](gqa_attention_base.py) · [grouped_query_attention.py](grouped_query_attention.py) · [stateful_kvcache_gqa.py](stateful_kvcache_gqa.py)

大模型自回归生成（Autoregressive Generation）将一次推理严格划分为 Prefill（预填充）和 Decode（逐字解码）两个阶段。KV Cache 是连接这两个阶段的核心数据结构：它将历史 Token 的 Key/Value 向量缓存下来，避免重复计算，将生成 N 个 Token 的总时间复杂度从 O(N^3) 降至 O(N^2)。本文从数学推导、张量流转、硬件瓶颈到工程实现（Static Cache / register_buffer / expand 广播机制），系统梳理两阶段推理的全部核心知识点。

## 目录

1. [为什么需要 KV Cache？O(N^3) 灾难与 O(N^2) 救赎](#1-为什么需要-kv-cache)
2. [Prefill 与 Decode 的核心区别](#2-prefill-与-decode-的核心区别)
3. [所有层的 KV Cache 与不可变性](#3-所有层的-kv-cache-与不可变性)
4. [训练 vs 预填充 vs 解码：三阶段对比](#4-训练-vs-预填充-vs-解码)
5. [Decode 阶段数据流转全过程](#5-decode-阶段数据流转全过程)
6. [吐字速度随上下文线性增长的理论与实证](#6-吐字速度随上下文线性增长)
7. [为什么是 Memory-Bound？算术强度分析](#7-为什么是-memory-bound)
8. [Static Cache 与 register_buffer](#8-static-cache-与-register_buffer)
9. [Tensor.expand() 的维度限制与 Stride 原理](#9-tensorexpand-的维度限制)
10. [torch.repeat_interleave() 用法详解](#10-torchrepeat_interleave-用法详解)
...
## 7. 为什么是 Memory-Bound？算术强度（Arithmetic Intensity）严谨推导
...
## 8. Static Cache 与 register_buffer 深度解析
...
## 9. Tensor.expand() 的维度限制与底层 Stride 原理
...
## 10. torch.repeat_interleave() 用法详解
...
本笔记所在目录 `Stateful_KV_GQA/` 包含以下文件：

| 文件 | 说明 |
|------|------|
| `grouped_query_attention.py` | GQA 动态缓存版本：每次 forward 通过 `torch.cat` 拼接历史 KV Cache，顶部注释引用 `stateful_kvcache_gqa.py` |
| `stateful_kvcache_gqa.py` | GQA 静态预分配缓存版本：`register_buffer` + `start_pos` 索引就地写入，顶部注释引用 `grouped_query_attention.py` |
| `kv_cache_prefill_decode_notes.md` | 本笔记，10 个专题覆盖 KV Cache / Prefill / Decode 全部核心知识点 |
| `README.md` | 目录索引与学习路线指南 |